# 就酱次元区 (Jiu Jiang Ci Yuan Qu)

## 一、项目概述

“就酱次元区”是一款面向粤港澳大湾区二次元爱好者的社交创作社区小程序。
...
- 正式登录页改为点击“微信登录”后弹出授权小窗，通过 `wx.login` 换取微信 `openid` 并复用已有会话；头像和昵称只作为资料信息维护，后端需配置真实 `WX_APP_ID/WX_APP_SECRET`
...
为适配当前审核要求，后台需要提供一个“发布入口总开关”，并以数据库全局配置控制：

- 数据表：`app_settings`
- 控制字段：`publish_enabled`
- 默认值：`0`（关闭）

控制规则：

- 当 `publish_enabled = 0` 时，小程序内所有“发布相关入口”默认隐藏或关闭。
- 当 `publish_enabled = 1` 时，统一放开所有“发布相关入口”。
...
- 该开关是“发布能力总控”，用于统一控制前端入口展示与后端发布能力放行。
- 后续 admin 后台实现时，默认按此 README 约定执行，避免前后端口径不一致。
